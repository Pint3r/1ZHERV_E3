...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
        ...
        {
            // shooting direction 
            Vector3 shootDir = new Vector3(Mathf.Cos(director.eulerAngles.z * Mathf.Deg2Rad),
                                           Mathf.Sin(director.eulerAngles.z * Mathf.Deg2Rad),
                                           0f);

            // set the start pos of bullets
            Vector3 basePos = director.position ;

            if (!shotgun)
            {
                // single shot
                SpawnBullet(basePos, Quaternion.Euler(0f, 0f, director.eulerAngles.z));
            }
            else
            {
                // shotgun
                ...
                //calculate all bullets
                for (int i = 0; i < shotgunBullets; i++)
                {
                    float angle = Mathf.Lerp(-halfSpread, halfSpread, (float)i / (shotgunBullets - 1));
                    float finalAngle = director.eulerAngles.z + angle;

                    Vector3 dir = new Vector3(Mathf.Cos(finalAngle * Mathf.Deg2Rad),
                                              Mathf.Sin(finalAngle * Mathf.Deg2Rad),
                                              0f);

                    Vector3 pos = director.position ;

                    SpawnBullet(pos, Quaternion.Euler(0f, 0f, finalAngle));
                }
            }
